...
const signup = catchAsync(async (req, res, next) => {
  const newUser = await UserModel.create({
    // we cant create a payload directly from the user browser
    name: req.body.name,
...
const forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user)
    return next(new AppError("Email does not exists in the database", 404)); // but this way someone can know if the email that they have is valid or not
  ...
  try {
    ...
    return next(new AppError("Something went wrong in server side", 500));
  }
